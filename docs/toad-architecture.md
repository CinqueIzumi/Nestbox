...
TOAD is Nestbox's home-grown unidirectional (MVI-style) presentation architecture. Read this
before adding a feature so you can follow the pattern without reverse-engineering existing code.

The base classes live in `core/presentation/toad/`. Every feature is built from the same five
type parameters, threaded through `ToadScreenModel<S, E, D, F, V>`:
...
Each feature lives under `feature/<name>/` with this layout (see `feature/profile/` for a worked
example that integrates a `core` repository):
...
4. **Action** — `sealed interface XAction : UiAction<XDependencies, XUiState, XEvent,
   ...
   is a `data object` in its **own file**, named with the `Action` suffix (`StartGitHubLinkAction.kt`),
...
8. **DI** — `val xModule = module { factory { XScreenModel(get(), ..., flows =
   listOf(SomeCollector)) } }`. Register it in `NestboxModule.kt`'s `nestboxModules`. **Pass
   collectors directly into the factory — do _not_ bind them as `single<List<XCollector>>`.**
...
9. **Navigation** — if it's a bottom-bar destination, add an `XTab : Tab` and list it in
   `core/presentation/widget/BottomFloatingBar.kt`.
...
- Put cross-feature state (auth, session, anything more than one feature reads) in a `core`
  ...
  with a collector — don't trap shared state inside a feature. The GitHub link state
  (`core/data/auth/AccountRepository`) is the reference example.
